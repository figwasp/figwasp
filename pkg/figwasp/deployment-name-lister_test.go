...
func TestDeploymentNameLister(t *testing.T) {
	var (
		e error
	)
	...
	const (
		deploymentLabelKey   = "figwasp/target"
		deploymentLabelValue = "true"
	...
	deployment0, e = deployments.NewKubernetesDeployment(
		...
		deployments.WithLabel(deploymentLabelKey, deploymentLabelValue),
	...
	var (
		config *rest.Config
		lister *labelSelectorDeploymentNameLister
	...
	lister, e = NewLabelSelectorDeploymentNameLister(config,
		v1.NamespaceDefault,
		fmt.Sprintf(labelSelectorFormat,
			deploymentLabelKey,
	...
	deploymentNames, e = lister.ListDeploymentNames(
		context.Background(),
	)
	if e != nil {
		t.Error(e)
	...
	assert.ElementsMatch(t,
		deploymentNames,
		[]string{deploymentName0, deploymentName1},
	)
}
